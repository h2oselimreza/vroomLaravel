...
<div class="btn-group btn-group-lg hidden-xs btn-group-justified" role="group" aria-label="...">
    ...
    @else
        <div class="btn-group" role="group">
            <a href="{{ isset($data) ? route('client.vendor.venor-list.edit', $data->id) : '#' }}">
    ...
    <div class="btn-group" role="group">
        <a href="{{ isset($data) ? route('client.employee.office.edit', $data->id) : '#' }}">
            <button type="button" onclick="employeeRoute('official')" class="btn btn-<?php echo ($secondLastSegment == 'office') ? 'info' : 'default' ?> waves-effect btn-lg" <?php echo ($disableFlag == '1') ? 'disabled' : '' ?>> <i class="fa fa-home"></i><b> Images</b></button>
        </a>
    </div>
    <div class="btn-group" role="group">
        <a href="{{ isset($data) ? route('client.employee.photograph.edit', $data->id) : '#' }}">
            <button type="button" onclick="employeeRoute('photograph')" class="btn btn-<?php echo ($secondLastSegment == 'photograph') ? 'info' : 'default' ?> waves-effect btn-lg" <?php echo ($disableFlag == '1') ? 'disabled' : '' ?>> <i class="fa fa-image"></i><b> Attachment</b></button>
        </a>
    </div>
</div>