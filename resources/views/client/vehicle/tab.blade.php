    ...
    @if ($thirdLastSegment == 'info')
        <div class="btn-group" role="group">
            <a href="{{ route('client.employee.create') }}">
                <button type="submit"  class="btn btn-<?php echo ($thirdLastSegment == 'info') ? 'info' : 'default' ?> btn-lg waves-effect"> <i class="fa fa-user"></i><b>  General Information</b></button>
            </a>
        </div>
    @else
        <div class="btn-group" role="group">
            <a href="{{ isset($data) ? route('client.vehicle.edit', $data->id) : '#' }}">
                <button type="submit"  class="btn btn-<?php echo ($secondLastSegment == 'create') ? 'info' : 'default' ?> btn-lg waves-effect"> <i class="fa fa-user"></i><b>  General Information</b></button>
            </a>
        </div>
    @endif
    <div class="btn-group" role="group">
        <a href="{{ isset($data) ? route('client.documentation.edit', $data->vehicle_id) : '#' }}">
            <button type="button" class="btn btn-<?php echo ($secondLastSegment == 'documentation') ? 'info' : 'default' ?> waves-effect btn-lg" <?php echo ($disableFlag == '1') ? 'disabled' : '' ?>> <i class="fa fa-home"></i><b> Vehicle Documentation</b></button>
        </a>
    </div>
</div>
<!-- end for web device -->
