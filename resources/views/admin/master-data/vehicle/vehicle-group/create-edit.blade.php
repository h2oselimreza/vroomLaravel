...
<div class="main-content">
    <div class="card from_card">
        ...
        <div class="card-body" x-data="vehicleForm()">
            <form 
                action="{{ isset($data->exists) ? route('admin.modules.master-data.vehicle-group.update', $data->id) : route('admin.modules.master-data.vehicle-group.store') }}"
                method="POST"
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" name="company" 
                            ...
                            @change="validateField('company')"
                            :class="errors.company ? 'is-invalid' : ''">
                            <option value="">Select company</option>
                            @if ($company)
                                @foreach ($company as $value)
                                    <option @php ($value->company_code == $value->company) ? 'selected':'' @endphp value="{{ $value->company_code }}">{{ $value->title }}</option>
                                @endforeach
                            @endif
                        </select>

                        <template x-if="errors.company">
                            <div class="text-danger small" x-text="errors.company"></div>
                        </template>
...
function vehicleForm() {
    return {
        formData: {
            ...
            company: "{{ old('company', $data->company ?? '') }}",
        },
        errors: {},
        isSubmitting: false,
