        ...
        <div class="col-sm-12 col-md-12">
            <div class="panel panel-default"> 
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <form action="{{ route('print.member.id.card') }}" method="post" class="me-2">
                        @csrf
                        <input type="hidden" name="member_ids" id="member_ids_card">
                        <input type="submit" class="btn btn-success save_button" value="Print Id Card">
                    </form>
                    <!-- Form 1: Print Member List -->
                    <form action="{{ route('print.member.id.card') }}" method="post">
                        @csrf
                        <input type="hidden" name="member_ids" id="member_ids_list">
                        <input type="hidden" name="isIdCard" value="1">
                        <input type="submit" class="btn btn-success save_button" value="Print Member List">
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    table = $('#datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.memberIdCard.data.index') }}",
        columns: [
    ...
    $('#member_ids_list').val(selected.join(','));
    $('#member_ids_card').val(selected.join(','));
});
