...
class HomeServiceCategoryController extends Controller
{
    public function index() {
        ...
        ->get();
        return view('admin.master-data.home-service.service-category.index',compact('data'));
    }

    public function create()
    {
        $categories = $this->getServiceCategory();
        return view('admin.master-data.home-service.service-category.create-edit',compact('categories'));
    }
     ...
     public function store(CostCategoryRequest $request, TokenService $tokenService)
    {
        ...
        $exists = ServiceCategory::where(['category_code'=>$data['parent_category'], 'category_type'=>'HOME'])
        ...
        try {
            ...
            if ($data['parent_category'] != 1) {
                $parent = ServiceCategory::where(['category_code'=>$data['parent_category'], 'category_type'=>'HOME'])->first();
                if ($parent) {
            ...
            return redirect()
                ->route('admin.modules.master-data.home-service-category.index')
    ...
    {
        $data = ServiceCategory::where(['category_code'=>$category_code,'category_type'=>'HOME'])->first();
        $categories = $this->getServiceCategory();
        //dd($data, $categories);
        return view('admin.master-data.home-service.service-category.create-edit',compact('categories','data'));
    }

    public function update(CostCategoryRequest $request, $category_code)
    {
        $cost_category = ServiceCategory::where(['category_code'=>$category_code,'category_type'=>'HOME'])->first();
        $data = $request->validated();

        $exists = ServiceCategory::where(['category_name'=>$data['category_name'],'category_type'=>'HOME'])
    ...
    {
        $data = ServiceCategory::where(['category_code'=>$code,'category_type'=>'HOME'])->firstOrFail();
