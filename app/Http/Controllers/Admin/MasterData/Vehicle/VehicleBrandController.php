...
class VehicleBrandController extends Controller
{
    ...
    {
        $data = $vehicleRepository->getVehicleBrand();
        return view('admin.master-data.vehicle.vehicle-brand.index',compact('data'));
    }
    ...
    {
        return view('admin.master-data.vehicle.vehicle-brand.create-edit'); 
    }
    ...
    public function store(VehicleTypeRequest $request, TokenService $tokenService)
    {
        ...
        try {
            ...
            return redirect()
                ->route('admin.modules.master-data.vehicle-brand.index')
                ->with('success', 'Vehicle brand created successfully!');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Vehicle brand Store Error: ' . $e->getMessage());
            return redirect()
                ->back()
                ->withInput()
    ...
    {
        $data = $vehicleRepository->getVehicleBrand($id);
        return view('admin.master-data.vehicle.vehicle-brand.show',compact('data'));
    }
    ...
    {
        $data = $vehicleRepository->getVehicleBrand($id);
        return view('admin.master-data.vehicle.vehicle-brand.create-edit',compact('data'));
    }
    ...
    {
        $vehicleType = CommonTable::where('type', 'vehicle_brand')->find($id);

        if (!$vehicleType) {
            return redirect()
                ->route('admin.modules.master-data.vehicle-brand.index')
                ->with('error', 'Vehicle brand not found!');
        }
        ...
        try {
            ...
            DB::rollBack();
            Log::error("Vehicle brand Update Failed (ID: $id): " . $e->getMessage());
